...
def pantalla_perfil(usuario):
    ...
    if usuario["deuda"] > 0:
        st.warning(
            f"⚠️ Tienes una deuda pendiente de L.{usuario['deuda']:.2f}. "
            "Para pagarla, dirígete a la ventanilla de la biblioteca. "
...
def pantalla_multa(usuario):
    ...
    if deuda <= 0:
        ...
        st.info("Puedes solicitar préstamos normalmente.")
        return

    st.metric("Deuda pendiente", f"L.{deuda:.2f}")
    st.warning(
        "⚠️ Tienes una multa pendiente. Para realizar el pago, "
        "dirígete a la ventanilla de la biblioteca."
    )
    st.info(
        "Mientras tengas una deuda pendiente no podrás solicitar nuevos préstamos. "
        "Cuando el bibliotecario registre tu pago, podrás volver a solicitar libros."
    )
...
def pantalla_usuarios_admin():
    ...
    if not codigos:
        ...
        deuda_clientes = [u for u in db.usuarios if u["rol"] == "CLIENTE" and float(u.get("deuda", 0) or 0) > 0]
        if not deuda_clientes:
            st.success("No hay clientes con deuda pendiente.")
        else:
            opciones_pago = {f"{u['codigo']} — {u['nombre']} — L.{u['deuda']:.2f}": u for u in deuda_clientes}
            with st.form("form_pago_ventanilla"):
                seleccion_pago = st.selectbox("Cliente", list(opciones_pago.keys()))
                cliente_pago = opciones_pago[seleccion_pago]
                monto_pago = st.number_input(
                    "Monto recibido (L.)",
                    min_value=0.01,
                    max_value=float(cliente_pago["deuda"]),
                    value=float(cliente_pago["deuda"]),
                    step=1.0,
                )
                registrar = st.form_submit_button("Registrar pago en ventanilla", type="primary")
            if registrar:
                ok, mensaje = db.registrar_pago(cliente_pago["codigo"], monto_pago)
                (st.success if ok else st.error)(mensaje)
                if ok:
                    st.rerun()
